...
HOST, PORT = '147.175.180.139', 5060  # potreba zadat vlastnej ip
...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def changeRequestUri(self):
        ...
        if md:
            method = md.group(1)
            uri = md.group(2)
            if uri in registrar:  # registrar.has_key(uri):
    ...
    def processRegister(self):
        ...
        if rx_invalid.search(contact) or rx_invalid2.search(contact):
            if fromm in registrar:  # registrar.has_key(fromm):
                del registrar[fromm]
        ...
        if expires == 0:
            if fromm in registrar:  # registrar.has_key(fromm)
                del registrar[fromm]
                self.sendResponse("200 super hej okey")
                return
        else:
            now = int(time.time())
    ...
    def processInvite(self):
        ...
        if len(origin) == 0 or origin not in registrar:  # registrar.has_key(origin):
